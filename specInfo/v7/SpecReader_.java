...
public class SpecReader_ implements ExtendedPlugInFilter {
	...
	public void run(ImageProcessor ip){
		...
		if(stack.getSize() > 1){
	  	...
	  	// get log file
	  	String path = "";
	  	if (call <=1) { path = getSpecFile(fi);}
	   	// parse to find metadata for the image
	   	IJ.log(path);
	  	infoArray = parseSpecFile(path,impTitle); 
//	  	IJ.log(""+infoArray[1]);
/*
	...
	/* Methods */
   	...
   	public String getSpecFile(FileInfo fi){
   	
			// Get logfile, by searching current image directory
			File file = new File("");
			...
			// If logfile is not found prompt the user to find it	
				IJ.error("log file not found");
				String path = IJ.getFilePath("Locate .alog file");
				return path;
			} else {
	...
	public String[] parseSpecFile(String filePath, String impTitle){
		try {
		ArrayList list = new ArrayList(50); // 50 is # of lines to read from log file for imp
		File file = new File(filePath);
		// Scan the  logfile for the title of the image, when it matches take the following 50 lines
