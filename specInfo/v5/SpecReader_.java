...
public class SpecReader_ implements ExtendedPlugInFilter {
	...
	public int showDialog(ImagePlus imp, String command, PlugInFilterRunner pfr) {
		GenericDialog gd = new GenericDialog("Spec Info");
        	...
        	gd.addCheckbox("Pull up Info",true);
        	gd.setOKLabel("Yes");
	...
	public void run(ImageProcessor ip){
		...
		if(stack.getSize() > 1){
			...
			if(call <= 1){
				impTitle = stack.getSliceLabel(slice);	
			} else {
				impTitle = stack.getSliceLabel(call);
			}
		} else {
	...
	//   	IJ.log(""+fi.directory);
	  ...
	  // 	String specFilePath = IJ.getFilePath("Locate .alog file");
	   	// Get path of spec file *.hlog
//	   	specFilePath = getSpecFilePath(fi,impTitle);	
...
/*	  	// And this is how you access them now or later
...
/*	
 ...
 /*  	public String getSpecFilePath (FileInfo fi, String impTitle){
  /* 		// Checks for logfile in the same dir as image
		String regexp = "[.]+";
		String[] parts = impTitle.split(regexp);
		String specFileName = parts[0]+".alog";
	//	String specFileName = "BFO_STO_align.alog";
		String specFilePath = fi.directory + specFileName;
		// Check if file exists, if not prompt for new path
		File file;
		file = new File(specFilePath);
		if (!file.exists()) {
			IJ.error(".alog file not found in same folder as image.");
			specFilePath = IJ.getFilePath("Locate .alog file");
		}
		*/
...
*/
	...
	public String[] parseSpecFile(FileInfo fi, String impTitle){
	//	File file;
		File f = new File(fi.directory);
		File[] matchingFiles = f.listFiles(new FilenameFilter() {
    	public boolean accept(File dir, String name) {
		...
		ArrayList list = new ArrayList(50); // 70 is # of lines to read from hlog file for imp
			...
			while (scanner.hasNextLine()) {
   				...
   				if(lineFromFile.contains(impTitle)) {
   					...
   					for(int i=1; i<50; i++){ 
       						...
       						// while scanning see if the user's fieldLabel is found
       					//	if(lineFromFile.contains(fieldLabel)){
       					//		
       					//	}
   					}
       					break;
